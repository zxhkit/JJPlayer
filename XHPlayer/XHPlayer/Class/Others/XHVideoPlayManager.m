...
/** 播放记录 */
@property (nonatomic, strong) NSMutableArray<XHVideoPlayTimeRecord *> *playRecords;
...
@implementation XHVideoPlayManager

ServiceSingletonM(XHVideoPlayManager)
...
- (NSMutableArray<XHVideoPlayTimeRecord *> *)playRecords{
...
- (void)recordUrl:(NSString *)url playTime:(float)playTime{
    ...
    XHVideoPlayTimeRecord * record = [self recordWithUrl:url];
    if(!record){
        record = [[XHVideoPlayTimeRecord alloc] init];
    }
    record.url = url;
...
- (XHVideoPlayTimeRecord *)recordWithUrl:(NSString *)url{
    ...
    __block XHVideoPlayTimeRecord * record = nil;
    [self.playRecords enumerateObjectsUsingBlock:^(XHVideoPlayTimeRecord * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)record:(XHVideoPlayTimeRecord *)record{
