...
/** 缓冲进度条 */
@property (nonatomic, strong) XHBufferSlider *bufferSlider;
/** 是否仅显示slider */
...
@implementation XHVideoBottomView
...
{
    if (!_fullScreenBtn) {
        _fullScreenBtn = [XHViewFactory buttonWithNormalImage:imgFullScreen selectedImage:imgHalfScreen];
...
- (XHBufferSlider *)bufferSlider
{
    if (!_bufferSlider) {
        _bufferSlider = [[XHBufferSlider alloc]initWithFrame:CGRectMake(0, -kSliderHeight/2.f, CGRectGetWidth(self.frame), kSliderHeight)];
...
- (void)setProgress:(CGFloat)progressValue{
    ...
    [self.playTimeLbl setText:[NSString stringWithFormat:@"%@",[XHVideoTimeUtil hmsStringWithFloat:progressValue]]];
}
...
- (void)setMaximumValue:(CGFloat)value{
    ...
    [self.totalDurationLbl setText:[NSString stringWithFormat:@"%@",[XHVideoTimeUtil hmsStringWithFloat:value]]];
}
