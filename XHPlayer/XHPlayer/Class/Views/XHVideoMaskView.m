...
#import "XHCircleLoading.h"
#import "XHVideoConst.h"
#import "XHViewFactory.h"

@interface XHVideoMaskView ()
...
/** 快进视图 */
@property (nonatomic, strong) XHFastForwardView *fastForwardView;
/** 加载视图 */
@property (nonatomic, strong) XHCircleLoading *circleLoading;
...
@implementation XHVideoMaskView
...
{
    if (!_playBtn) {
        _playBtn = [XHViewFactory buttonWithNormalImage:imgPlay selectedImage:imgPause];
...
- (XHFastForwardView *)fastForwardView{
    if (!_fastForwardView) {
        _fastForwardView = [[XHFastForwardView alloc] init];
...
- (XHCircleLoading *)circleLoading{
    if (!_circleLoading) {
        _circleLoading = [[XHCircleLoading alloc] init];
